...
void CGroupBox::Draw(bool hover)
{
	...
	Render::Text(a.x + (m_iWidth / 2) - (txtSize.right / 2), a.y - (txtSize.bottom / 2) - 1, Color(244, 244, 244, 255), Render::Fonts::MenuText, Text.c_str());
...
void CSlider1::OnUpdate() {
	...
	if (DoDrag)
	{
		...
		Render::Outline(a.x - 1, a.y + 4, m_iWidth + 2, 7, Color(140, 255, 0, 255));
	}
}
...
void CSlider025::Draw(bool hover)
{
	int red;
	int green;
	int blue;
...
float CSlider025::GetValue()
{
	return Value;
}

void CSlider025::SetValue(float v)
{
	Value = v;
}

void CSlider025::SetBoundaries(float min, float max)
{
	Min = min; Max = max;
}
...
void CKeyBind::Draw(bool hover)
{
	...
	Render::Text(a.x + 4, a.y + 2 - 1, Color(244, 244, 244, 255), Render::Fonts::MenuText, KeyName);
}
...
void CComboBox::Draw(bool hover)
{
	...
	Render::Clear(a.x, a.y, m_iWidth, 16, Color(60, 60, 60, 255));
